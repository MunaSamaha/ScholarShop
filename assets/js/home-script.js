 ...
 let productItems = document.querySelectorAll('.trending-shops');
  for(let i =0;i<=productItems.length-1;i++){
    if(i>3){
     productItems[i].classList.add('hide-trending-shops');
    }
  }
...
seeMoreBtn.onclick = function(e){
 ...
 for(let i = 0;i<=productItems.length;i++){
    if(productItems[i].classList.contains('hide-trending-shops')){
        productItems[i].classList.remove('hide-trending-shops');
        count++;
        totalItems++;
    }
    ...
    if(totalItems==productItems.length){
